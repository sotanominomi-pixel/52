document.addEventListener('DOMContentLoaded', () => {
  ...
  const L = {
    ja:{ 'tab.clock':'時計','tab.stopwatch':'ストップウォッチ','tab.alarm':'アラーム','tab.settings':'設定',
          ...
          'label.hours':'1日の長さ','msg.pickTime':'時刻を選択してください','msg.invalidTime':'不正な時刻です','msg.alarmSound':'アラームが鳴りました',
          'settings.showSeconds':'秒数表示','settings.language':'言語','footer':'設定は自動で保存されます','lap.none':'ラップなし','alarm.none':'アラームなし' },
    en:{ 'tab.clock':'Clock','tab.stopwatch':'Stopwatch','tab.alarm':'Alarm','tab.settings':'Settings',
          ...
          'label.hours':'Day length','msg.pickTime':'Please pick a time','msg.invalidTime':'Invalid time','msg.alarmSound':'Alarm',
          'settings.showSeconds':'Show seconds','settings.language':'Language','footer':'Settings are saved automatically','lap.none':'No laps','alarm.none':'No alarms' }
  };
  ...
  let showSeconds = (localStorage.getItem('nclock_show_seconds')==='true');
  ...
  function renderLaps(){
    lapList.innerHTML='';
    if(laps.length===0){
      lapList.innerHTML=`<div style="color:var(--muted);padding:8px">${t('lap.none')}</div>`;
      return;
    }
    laps.forEach((t,i)=>{
  ...
  swStart.addEventListener('click',()=>{
    ...
    if(running) lastFrame=performance.now(); else saveAll();
  });
  ...
  function renderAlarms(){
    ...
    if(alarms.length===0){
      alarmsContainer.innerHTML=`<div style="color:var(--muted);padding:8px">${t('alarm.none')}</div>`;
      return;
    }
